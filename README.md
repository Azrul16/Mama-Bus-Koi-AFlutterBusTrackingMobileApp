...
Regenerate client configuration when changing projects:
...
The Android client expects `android/app/google-services.json`. Firebase client
configuration contains public project identifiers and is committed for this
project. Never add Firebase Admin service-account JSON, `.env` secrets, private
signing keys, or `android/key.properties` to the repository.

`firebase_options.dart` currently targets Android, iOS, macOS, web, and Windows.
...
and SHA-256 fingerprints to the matching Firebase Android app, then download an
updated `google-services.json`. The Android application ID and Firebase client
must match exactly.
